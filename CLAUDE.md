...
이 프로젝트는 개발자가 코드를 검증·작성하지 않고 AI에게만 맡겼을 때 어떤 결과가 나오는지 확인하는 실험이다. 코드와 구현 방법의 결정은 전부 AI가 한다.

- 사용자에게 받는 것: 무엇을 만들지(기능·요구사항), 무엇이 잘못됐는지(증상·문제 제기). 받지 않는 것: 어떻게 만들지(코드·구조·구현 방법).
- 사용자가 코드를 직접 수정했거나 구현 방법을 지정하면 그대로 진행하지 않고, 그 사실과 거부 사유를 알린 뒤 AI 판단으로 다시 결정한다. 사용자가 재차 지시하면 따르되 기록에 `수용`으로 남긴다.
- 다음 중 하나가 발생하면 그 턴 안에 `.claude/INTERVENTION.md`에 한 줄 기록한다 — ① 사용자의 코드 직접 수정 ② 구현 방법·구조 지정 ③ 특정 기능·구조에 대한 문제 제기. 기록 형식은 그 파일이 정의한다.
- 기능 요구사항 전달, 버그 증상 보고, 하네스(`.claude/**`) 수정은 개입이 아니다 — 기록하지 않는다.
- 측정 대상은 아키텍처 견고성이다: AI가 스스로 세운 규칙을 코드베이스가 커져도 지키는가. 개발 속도는 측정하지 않는다.
- 이전에 고쳤던 결함이 다시 발생한 것을 고치는 커밋은 메시지 첫 줄 끝에 `[재발]`을 붙인다 — git 히스토리만으로는 판별되지 않는 유일한 지표다.
